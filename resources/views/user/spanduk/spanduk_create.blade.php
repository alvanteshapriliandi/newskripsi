  ...
  <!-- Page Content -->
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-md-5">
          <img class="img-fluid" src="{{ asset('uploads/'.$spanduk->images)}}" alt="" width="400">
        </div>

        <div class="col-md-4">
          <h3 class="my-3">Deskripsi Produk</h3>
          <p>{{$spanduk->description}}</p>
          <h3 class="my-3">Harga Produk</h3>
          <ul>
            <li>Rp. {{$spanduk->hrg_awal}},-</li>
          </ul>
        </div>
      ...
      <div class="row">
        <div class="col-lg-12">
          <form class="form-group" action="{{route('spanduk.store')}}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            <table class="table table-responsive">
              ...
              <tr>
                <th>Ukuran</th>
                <td>
                  <input type="text" name="BN_ukuran" placeholder="Ex : 100 X 100 CM" class="form-control">
                </td>
                <th>Jenis</th>
                <td>
              ...
              <tr>
                <th colspan="4">
                  <input type="hidden" name="productid" value="{{$spanduk->id}}">
                  @if(!Auth::guest())
                    @if(Auth::user()->role == 1)
