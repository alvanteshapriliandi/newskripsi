...
@section('content')
    ...
    <div class="container">
      ...
      <h1 class="my-4">{{$kartunama ->jdl_Pdk}}
      </h1>
      ...
      <div class="row">
        ...
        <div class="col-md-5">
          <img class="img-fluid" src="{{ asset('uploads/'.$kartunama->images)}}" alt="" width="400">
        </div>

        <div class="col-md-4">
          <h3 class="my-3">Deskripsi Produk</h3>
          <p>{{$kartunama->description}}</p>
          <h3 class="my-3">Harga Produk</h3>
          <ul>
            <li>Rp. {{$kartunama->hrg_awal}},-</li>
          </ul>
        </div>

        <div class="col-md-3" style="border: 2px solid #ccc;">
      ...
      <div class="row">
        <div class="col-lg-12">
          <form class="form-group" action="{{route('kartunama.store')}}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            <table class="table table-responsive">
              ...
              <tr>
                <th colspan="4">
                  <input type="hidden" name="productid" value="{{$kartunama->id}}">
                  @if(!Auth::guest())
                    @if(Auth::user()->role == 1)
