  ...
  <!-- Page Content -->
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-md-5">
          <img class="img-fluid" src="{{ asset('uploads/'.$amplop->images)}}" alt="" width="400">
        </div>

        <div class="col-md-4">
          <h3 class="my-3">Deskripsi Produk</h3>
          <p>{{$amplop->description}}</p>
          <h3 class="my-3">Harga Produk</h3>
          <ul>
            <li>Rp. {{$amplop->hrg_awal}},-</li>
          </ul>
        </div>
      ...
      <div class="row">
        <div class="col-lg-12">
          <form class="form-group" action="{{route('amplop.store')}}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            <table class="table table-responsive">
              ...
              <tr>
                <th colspan="4">
                  <input type="hidden" name="productid" value="{{$amplop->id}}">
                  @if(!Auth::guest())
                    @if(Auth::user()->role == 1)
